-- Complete Database Setup for ScanCard App
-- This script creates all necessary tables and ensures all migrations are applied
...
-- Create `events` table for storing event data (with image column included)
...
-- Verify that the image column exists in events table
DO $$ 
BEGIN
    IF NOT EXISTS (
        SELECT 1 
        ...
        RAISE EXCEPTION 'Image column is missing from events table. Please run the migration_add_event_image.sql script.';
    ELSE
        RAISE NOTICE 'Image column exists in events table - setup complete!';
    END IF;
END $$; 